...
class CRM_Report_Form_Activity extends CRM_Report_Form {
  ...
  function select() {
    ...
    /**
      ...
      * @date 24 June 2014
      * 
      * @todo Set column header at the end
      */
    $this->_columnHeaders['operations'] = array('title' => '');
  ...
  function alterDisplay(&$rows) {
    ...
    foreach ($rows as $rowNum => $row) {
      ...
      /**
        ...
        * @date 24 June 2014
        * 
        * @todo set link in field
        */
      if(!empty($atype) and !empty($rows[$rowNum]['civicrm_activity_id']) and !empty($cid)){
        if ($viewLinks) {
          $url = CRM_Utils_System::url("civicrm/activity/add",
            'atype=' . $atype . '&action=update&reset=1&id=' . $rows[$rowNum]['civicrm_activity_id'] . '&cid=' . $cid . '&context=activity',
            $this->_absoluteUrl
          );
          
          $rows[$rowNum]['operations'] = "<a title='" . $onHover . "' href='" . $url . "'>" . ts('edit') . "</a>";
        }
      }else {
        $rows[$rowNum]['operations'] = '';
      }
