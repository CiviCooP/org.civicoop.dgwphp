...
class CRM_Activity_Page_AJAX {
  static function getCaseActivity() {
    ...
    // get the attachements for every activity
    foreach($activities as $activity_id => $activty){
      $attachment_params = array(
        'version' => 3,
        'sequential' => 1,
        'activity_id' => $activity_id,
      );
      $attachment_result = civicrm_api('ActivityAttachments', 'get', $attachment_params);
      
      if(0 >= $attachment_result['count']){
        $activities[$activity_id]['attachments'] = ts('N');
      }else {
        $activities[$activity_id]['attachments'] = ts('J');
      }
    }
    
    $iFilteredTotal = $iTotal = $params['total'];
    $selectorElements = array('display_date', 'subject', 'type', 'with_contacts', 'reporter', 'status', 'attachments', 'links', 'class');
